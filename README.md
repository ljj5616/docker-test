...
| --- | --- | --- |
| PORT | 3000 | HTTP 포트 |
| HOST | 0.0.0.0 | 모든 IPv4 인터페이스에서 접속 수신 |
...
기본 서비스는 외부 접속을 받습니다. EC2 보안 그룹에서 TCP 3000 포트를 접속할 PC의 IP에 허용한 뒤 `http://EC2공인IP:3000`으로 접속하세요. `0.0.0.0`은 수신 설정이며 브라우저에 입력할 주소가 아닙니다.

로컬 접속만 허용하려면 `HOST=127.0.0.1`로 설정하고 SSH 터널로 접속할 수 있습니다.
